...
export const useWishlistStore = defineStore('wishlist', {
  ...
  getters: {
    ...
    filteredItems: (state) => {
      ...
      return state.items.filter(item => item.wishlist_category_id === state.selectedCategory.id)
    },
  ...
  actions: {
    ...
    async loadCategories() {
      ...
      try {
        ...
        this.error = error.response?.data?.message || 'Failed to load wishlist categories'
    ...
    async loadItems() {
      ...
      try {
        ...
        this.error = error.response?.data?.message || 'Failed to load wishlist items'
        this.handleError(this.error)
        return { success: false, error: this.error }
      } finally {
        this.loading = false
    ...
    async getDefaultCategory() {
      ...
      try {
        const response = await axios.get('/wishlist-categories/default')

        const defaultCategory = response.data.data

        // Add to categories if not exists
        const existingIndex = this.categories.findIndex(cat => cat.id === defaultCategory.id)
        if (existingIndex === -1) {
          this.categories.push(defaultCategory)
        } else {
          this.categories[existingIndex] = defaultCategory
        }

        return { success: true,data:  response.data }
      } catch (error) {
        this.error = error.response?.data?.message || 'Failed to get default category'
        this.handleError(this.error)
        return { success: false, error: this.error }
      } finally {
        this.loading = false
    ...
    async createDefaultCategory() {
      ...
      try {
        ...
        this.error = error.response?.data?.message || 'Failed to create default category'
        this.handleError(this.error)
        return { success: false, error: this.error }
      } finally {
        this.loading = false
    ...
    async updateCategory(categoryId, name, isDefault = false) {
      ...
      try {
        ...
        const response = await axios.post(`/wishlist-categories/${categoryId}`, data)
    ...
    async deleteCategory(categoryId) {
      ...
      try {
        ...
        this.items = this.items.filter(item => item.wishlist_category_id !== categoryId)
      ...
      try {
        ...
        if (!targetCategoryId) {
          if (this.defaultCategory) {
            ...
            targetCategoryId = defaultCat.data?.data?.id
          }
        }
        ...
        // Add to items array if not exists
        const existingItemIndex = this.items.findIndex(item => item.id === response.data.data.id)
        if (existingItemIndex === -1) {
          this.items.push(response.data.data)
        } else {
          this.items[existingItemIndex] = response.data.data
        }
    ...
    async moveItem(itemId, newCategoryId) {
      ...
      try {
        const response = await axios.post(`/wishlist-items/${itemId}/move`, {
        ...
        this.error = error.response?.data?.message || 'Failed to move item'
        this.handleError(this.error)
        return { success: false, error: this.error }
      } finally {
        this.loading = false
    ...
    async checkProductInWishlist(productId) {
      ...
      try {
        ...
        this.error = error.response?.data?.message || 'Failed to check wishlist status'
        this.handleError(this.error)
        return { success: false, error: this.error }
      } finally {
        this.loading = false
    ...
    selectCategory(categoryId) {
      this.selectedCategory = categoryId ? this.getCategoryById(categoryId) : null
    ...
    async initializeWishlist() {
      ...
      try {
        ...
        this.error = error.response?.data?.message || 'Failed to initialize wishlist'
        this.handleError(this.error)
        return { success: false, error: this.error }
      }
    },
