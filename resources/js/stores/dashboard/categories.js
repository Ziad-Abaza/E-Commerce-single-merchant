...
export const useCategoriesStore = defineStore("dashboardCategories", {
    state: () => ({
        ...
        categoryAttributes: [], // State for attributes of a specific category
        loading: false,
        deleting: false,
        error: null,
        pagination: {
    ...
    actions: {
        async fetchCategories(page = 1, perPage = 10) {
            ...
            try {
                ...
                const response = await axios.get("/dashboard/categories", { params });
        ...
        async fetchCategory(id) {
            ...
            try {
                ...
                await this.fetchAttributesForCategory(id);
                return response.data.data;
            } catch (err) {
                this.error = err.response?.data?.message || "Failed to load category details";
                throw err;
            } finally {
                this.loading = false;
            }
        ...
        async fetchAttributesForCategory(categoryId) {
            ...
            this.error = null;
            try {
                const response = await axios.get(`/dashboard/categories/${categoryId}/attributes`);
                this.categoryAttributes = response.data.data;
            } catch (error) {
                this.error = "Failed to fetch attributes for category.";
        ...
        // New action to save/sync attributes to a category
        async syncAttributesToCategory(categoryId, attributesData) {
            this.loading = true;
            this.error = null;
            try {
                await axios.post(`/dashboard/categories/${categoryId}/attributes`, {
                    attributes: attributesData,
                });
                // Refresh the attributes for the category to show the changes
                await this.fetchAttributesForCategory(categoryId);
            } catch (error) {
                this.error = "Failed to sync attributes to the category.";
        ...
        async createCategory(categoryData) {
            ...
            try {
                ...
                if (categoryData.is_active !== undefined) {
                    categoryData.is_active = categoryData.is_active === true || categoryData.is_active === "true" ? 1 : 0;
                }
                if (categoryData.parent_id !== undefined) {
                    categoryData.parent_id = categoryData.parent_id ? parseInt(categoryData.parent_id) : null;
                }
                for (let key in categoryData) {
                    if (key === "thumbnail" && categoryData[key] instanceof File) {
                        formData.append("thumbnail", categoryData[key]);
                    } else if (key === "icon" && categoryData[key] instanceof File) {
                        formData.append("icon", categoryData[key]);
                    } else if (key !== "thumbnail" && key !== "icon") {
                        formData.append(key, categoryData[key]);
                    }
                }
                const response = await axios.post("/dashboard/categories", formData, {
        ...
        async updateCategory(id, categoryData) {
            ...
            try {
                ...
                const response = await axios.post(`/dashboard/categories/${id}`, formData, {
                    headers: { "Content-Type": "multipart/form-data" },
                });
