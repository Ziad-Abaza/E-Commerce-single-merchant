...
export const usePromoCodesStore = defineStore("promoCodes", {
    ...
    actions: {
        ...
        // ============= TRASHED =============
        async fetchTrashedPromoCodes(page = 1, perPage = 15) {
            this.loading = true;
            this.error = null;
            try {
                const params = {
                ...
                const response = await axios.get(
                    "/dashboard/promo-codes/trash",
                    {
                        params,
                    },
                );
                this.trashedPromoCodes = response.data.data || [];
                ...
                this.error =
                    err.response?.data?.message ||
                    "Failed to fetch trashed promo codes";
                return { success: false, error: this.error };
            } finally {
                this.loading = false;
            }
        ...
        async restorePromoCode(id) {
            ...
            try {
                const response = await axios.post(
                    `/dashboard/promo-codes/${id}/restore`,
                );
                this.trashedPromoCodes = this.trashedPromoCodes.filter(
                    (p) => p.id !== id,
                );
                this.promoCodes.unshift(response.data.data);
                return { success: true, data: response.data.data };
        ...
        async forceDeletePromoCode(id) {
            this.deleting = true;
            this.error = null;
            try {
                await axios.delete(`/dashboard/promo-codes/${id}/force-delete`);
                this.trashedPromoCodes = this.trashedPromoCodes.filter(
                    (p) => p.id !== id,
                );
                return { success: true };
                ...
                this.error =
                    err.response?.data?.message ||
                    "Failed to permanently delete promo code";
                return { success: false, error: this.error };
            } finally {
                this.deleting = false;
            }
