  ...
  <div class="p-2 sm:p-4 md:p-6 space-y-4 sm:space-y-6">
    ...
    >
      <div class="flex items-center">
        ...
        <span class="text-sm text-red-700 dark:text-red-300 flex-1">{{ usersStore.error }}</span>
        <button
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
      ...
      <div class="bg-white dark:bg-gray-800 rounded-lg p-3 shadow-sm border border-gray-200 dark:border-gray-700 hover:shadow-md transition-all duration-300">
        <div class="flex items-center justify-between">
          <div>
            <p class="text-gray-500 dark:text-gray-400 text-xs font-medium">Total Users</p>
      ...
      <div class="bg-white dark:bg-gray-800 rounded-lg p-3 shadow-sm border border-gray-200 dark:border-gray-700 hover:shadow-md transition-all duration-300">
        <div class="flex items-center justify-between">
          <div>
            <p class="text-gray-500 dark:text-gray-400 text-xs font-medium">Trashed</p>
      ...
      <div class="bg-white dark:bg-gray-800 rounded-lg p-3 shadow-sm border border-gray-200 dark:border-gray-700 hover:shadow-md transition-all duration-300">
        <div class="flex items-center justify-between">
          <div>
            <p class="text-gray-500 dark:text-gray-400 text-xs font-medium">Verified</p>
      ...
      <div class="bg-white dark:bg-gray-800 rounded-lg p-3 shadow-sm border border-gray-200 dark:border-gray-700 hover:shadow-md transition-all duration-300">
        <div class="flex items-center justify-between">
          <div>
            <p class="text-gray-500 dark:text-gray-400 text-xs font-medium">Unverified</p>
    ...
    <!-- Loading Skeleton -->
    <div v-if="usersStore.loading && !usersStore.users.length" class="space-y-4">
      ...
      <div class="bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
        <div class="p-6">
          ...
          <div v-if="currentUser" class="space-y-6">
            ...
            <div class="bg-gray-50 dark:bg-gray-700/50 rounded-xl p-5">
              <div class="flex flex-col sm:flex-row gap-4">
                <div class="flex-shrink-0">
                  <img
                    :src="currentUser.avatar_url || '/images/placeholder-user.png'"
                    :alt="currentUser.name"
                    class="w-24 h-24 object-cover rounded-full"
                  />
                </div>
                <div class="flex-1">
                  <h4 class="text-lg font-bold text-gray-900 dark:text-white">{{ currentUser.name }}</h4>
                  <p class="text-gray-600 dark:text-gray-400">{{ currentUser.email }}</p>
                  <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">{{ currentUser.phone || 'No phone number' }}</p>
                  <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">{{ currentUser.address || 'No address' }}</p>
                  <div class="flex flex-wrap gap-2 mt-3">
                    <span
                      :class="[
                        'inline-flex items-center px-2 py-1 rounded-full text-xs font-medium',
                        currentUser.is_active ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300' : 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-300'
                      ]"
                    >
                    ...
                    <span
                      :class="[
                        'inline-flex items-center px-2 py-1 rounded-full text-xs font-medium',
                        currentUser.email_verified_at ? 'bg-purple-100 text-purple-800 dark:bg-purple-900/30 dark:text-purple-300' : 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-300'
                      ]"
                    >
            ...
            <!-- User Details -->
            <div class="bg-gray-50 dark:bg-gray-700/50 rounded-xl p-5">
              <h4 class="font-semibold text-lg text-gray-900 dark:text-white mb-4">Account Information</h4>
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="space-y-2">
                  <label class="block text-sm font-medium text-gray-500 dark:text-gray-400">User ID</label>
                  <p class="text-sm font-medium text-gray-900 dark:text-white">{{ currentUser.id }}</p>
                </div>
                <div class="space-y-2">
                ...
                <div class="space-y-2">
                  ...
                  <p class="text-sm font-medium text-gray-900 dark:text-white">{{ currentUser.phone || 'N/A' }}</p>
                </div>
                <div class="space-y-2">
                  <label class="block text-sm font-medium text-gray-500 dark:text-gray-400">Address</label>
                  <p class="text-sm font-medium text-gray-900 dark:text-white">{{ currentUser.address || 'N/A' }}</p>
                </div>
                <div class="space-y-2">
                  <label class="block text-sm font-medium text-gray-500 dark:text-gray-400">Created At</label>
                  <p class="text-sm font-medium text-gray-900 dark:text-white">{{ formatDate(currentUser.created_at) }}</p>
                </div>
                <div class="space-y-2">
                  <label class="block text-sm font-medium text-gray-500 dark:text-gray-400">Last Updated</label>
                  <p class="text-sm font-medium text-gray-900 dark:text-white">{{ formatDate(currentUser.updated_at) }}</p>
                </div>
              </div>
            </div>
            ...
            <div class="bg-gray-50 dark:bg-gray-700/50 rounded-xl p-5">
              <div class="flex items-center justify-between mb-4">
                <h4 class="font-semibold text-lg text-gray-900 dark:text-white">User Roles</h4>
                <button
          ...
          <div class="space-y-4">
            ...
            <div class="space-y-3 max-h-60 overflow-y-auto">
              ...
              >
                <input
                  ...
                  @change="handleRoleChange(role.id, $event.target.checked)"
            ...
            <div class="flex justify-end gap-3 mt-6">
              ...
              >
                ...
                {{ usersStore.loading ? 'Saving...' : 'Save Roles' }}
              </button>
            </div>
          </div>
        </div>
...
if (!authStore.hasPermission('manage_users')) {
  throw new Error('Access denied: You do not have permission to manage users');
}
...
const fetchUsers = async () => {
  await usersStore.fetchUsers(usersStore.pagination.current_page, usersStore.pagination.per_page);
};
...
const initializeFormFields = (user) => {
  formFields.value = [
    ...
    {
      id: 'password',
      label: isEditing.value ? 'New Password (leave blank to keep current)' : 'Password',
      type: 'password',
      value: '',
      required: !isEditing.value,
      placeholder: isEditing.value ? 'Enter new password' : 'Enter password'
    },
    {
      id: 'password_confirmation',
      label: isEditing.value ? 'Confirm New Password' : 'Confirm Password',
      type: 'password',
      value: '',
      required: !isEditing.value,
      placeholder: isEditing.value ? 'Confirm new password' : 'Confirm password'
    },
    {
      id: 'is_active',
      label: 'Active Status',
...
const handleManageRoles = async (user) => {
  try {
    ...
    // OLD LINE: selectedRoles.value = user.roles ? user.roles.map(role => role.id) : [];
    // NEW LINE: Ensure role.id exists (it should now after store processing)
    selectedRoles.value = user.roles && Array.isArray(user.roles) ? user.roles.map(role => role.id).filter(id => id != null) : [];
...
const handleSaveRoles = async () => {
  ...
  try {
    ...
    if (currentUser.value && currentUser.value.id === userForRoles.value.id) {
      const updatedUser = await usersStore.fetchUser(userForRoles.value.id);
    ...
    usersStore.error = 'Failed to update user roles: ' + (error.response?.data?.message || error.message);
  }
};
