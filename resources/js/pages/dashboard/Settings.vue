  ...
  <div class="p-2 sm:p-4 md:p-6 space-y-4 sm:space-y-6">
    ...
    <div class="flex flex-col gap-4">
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <div class="flex flex-col sm:flex-row gap-2">
          ...
          >
            <svg v-if="settingsStore.saving" class="animate-spin -ml-1 mr-2 h-4 w-4 text-white" fill="none" viewBox="0 0 24 24">
    ...
    >
      <div class="flex items-center">
        ...
        <span class="text-sm text-red-700 dark:text-red-300 flex-1">{{ settingsStore.error }}</span>
        <button
    ...
    <div class="space-y-6">
      ...
      <div v-else-if="currentGroupSettings.length > 0" class="space-y-6">
        ...
        >
          <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
            <div class="flex-1">
              <label :for="`setting-${setting.id}`" class="block text-sm font-medium text-gray-900 dark:text-white mb-1">
                {{ setting.label }}
                <span v-if="setting._isDirty" class="text-orange-500 ml-1">*</span>
              </label>
              <p v-if="setting.description" class="text-xs text-gray-500 dark:text-gray-400 mb-3">
                {{ setting.description }}
              </p>
              ...
              <div v-else-if="setting.type === 'boolean'" class="flex items-center">
                ...
                />
                <label :for="`setting-${setting.id}`" class="ml-2 text-sm text-gray-900 dark:text-white">
                  Enable this setting
                </label>
              </div>
              ...
              <!-- File Upload -->
              <div v-else-if="setting.type === 'file' || setting.type === 'image'">
                <input
                  :id="`setting-${setting.id}`"
                  @change="handleFileUpload(setting, $event)"
                  type="file"
                  :accept="setting.type === 'image' ? 'image/*' : '*'"
                  class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-white"
                />
                <p v-if="setting.value" class="mt-2 text-sm text-gray-600 dark:text-gray-400">
      ...
      <div v-else class="text-center py-12">
        ...
        <h3 class="mt-2 text-sm font-medium text-gray-900 dark:text-white">No settings</h3>
...
if (!authStore.hasPermission('manage_settings')) {
  throw new Error('Access denied: You do not have permission to manage settings');
}
...
const handleSaveSetting = async (settingData) => {
  try {
    if (showEditModal.value) {
      await settingsStore.updateSetting(editingSetting.value.id, settingData);
    } else {
