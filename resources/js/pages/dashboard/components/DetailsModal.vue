...
<template>
  ...
  >
    ...
    >
      ...
      <div v-if="item" class="p-6">
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div class="lg:col-span-1">
            <div class="bg-gray-50 dark:bg-gray-700/50 rounded-xl p-4 h-full flex flex-col">
              <h4 class="font-semibold text-gray-900 dark:text-white mb-4 text-center">
                {{ imageSectionTitle }}
              </h4>
              <div class="flex-1 flex items-center justify-center">
                <template v-if="mainMedia && mainMedia.type === 'image'">
                  <img
                    :src="mainMedia.src"
                    :alt="item.name || 'Item'"
                    ...
                    @click="$emit('preview', mainMedia.full || mainMedia.src)"
                  />
                </template>
                <template v-else>
              ...
              <div v-if="hasGalleryImages" class="mt-4">
                ...
                <div class="flex space-x-2 overflow-x-auto pb-2">
                  <div
                    ...
                    @click="$emit('preview', image.url || image)"
                  >
                    <img
                      :src="image.url || image"
          ...
          <div class="lg:col-span-2 space-y-6">
            ...
            >
              <h4 class="font-semibold text-lg text-gray-900 dark:text-white mb-4 flex items-center">
                <svg
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                >
                  ...
                  <template v-if="field.type === 'boolean'">
                    <span
                      :class="[
                        ...
                        field.value ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300' : 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-300'
                      ]"
                    >
                  ...
                  <template v-else-if="field.type === 'array' && Array.isArray(field.value)">
                    <div class="flex flex-wrap gap-2">
                      ...
                      >
                        {{ typeof item === 'object' ? item.name || item.label || JSON.stringify(item) : item }}
                      </span>
                      <span v-if="field.value.length === 0" class="text-sm text-gray-500">No items</span>
                    </div>
                  </template>

                  <template v-else-if="field.type === 'object' && typeof field.value === 'object'">
                    <div class="bg-white dark:bg-gray-700 p-3 rounded-lg text-sm">
                      <pre class="whitespace-pre-wrap">{{ JSON.stringify(field.value, null, 2) }}</pre>
                    </div>
                  </template>
                  ...
                  <template v-else-if="field.type === 'image'">
                    <img
                      ...
                      @click="$emit('preview', field.value)"
                    />
                  </template>
                  ...
                  <template v-else>
                    <p class="text-sm font-medium text-gray-900 dark:text-white">
                      {{ field.value !== null && field.value !== undefined ? field.value.toString() : 'N/A' }}
                    </p>
                  </template>
                </div>
              </div>
      ...
      <!-- Modal Footer -->
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between p-6 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-700/50 rounded-b-xl">
...
const modalTitle = computed(() => {
  return props.title || (props.item?.name ? `${props.item.name} Details` : "Item Details");
});
...
const mainMedia = computed(() => {
  ...
  if (Array.isArray(props.item.gallery_images) && props.item.gallery_images.length > 0) {
  ...
  // Try images_url
  if (Array.isArray(props.item.images_url) && props.item.images_url.length > 0) {
    return {
      type: 'image',
...
const sections = computed(() => {
  ...
  const basicFields = [];
  const basicKeys = ['name', 'brand', 'sku', 'price', 'stock', 'stock_quantity', 'final_price', 'discount', 'discount_percentage'];
  ...
  const statusFields = [];
  const statusKeys = ['is_active', 'status', 'is_in_stock', 'is_low_stock', 'is_out_of_stock', 'min_stock_alert', 'stock_alert_level'];
  ...
  const productFields = [];
  const productKeys = ['size', 'color', 'material', 'weight', 'weight_in_grams', 'origin', 'quality', 'packaging', 'sku_variant', 'barcode'];
  ...
  if (props.item.dimensions && typeof props.item.dimensions === 'object') {
    ...
    dimensionKeys.forEach(key => {
      if (props.item.dimensions[key] !== undefined && props.item.dimensions[key] !== null) {
  ...
  const additionalKeys = ['created_at', 'updated_at', 'published_at', 'expires_at'];
  ...
  // Categories section
  if (props.item.categories && Array.isArray(props.item.categories) && props.item.categories.length > 0) {
    sections.push({
      title: 'Categories',
      fields: [{
        label: 'Categories',
