                ...
                <div class="p-6">
                    ...
                    >
                        ...
                        <div class="pt-2 border-t border-gray-100 dark:border-gray-700">
                            <div v-if="!cartStore.promoCode" class="flex items-center gap-2 mt-2">
                                <div class="flex-1">
                                    ...
                                    <input
                                        ...
                                        class="block w-full rounded-lg border-gray-300 dark:border-gray-600 shadow-sm focus:border-primary-500 focus:ring-primary-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-white text-sm p-2.5"
                                        :disabled="applyingPromo"
                                    />
                                    <p v-if="promoError" class="mt-1 text-sm text-red-600 dark:text-red-400">
                                        {{ promoError }}
                                    </p>
                                </div>
                                <button
                                    ...
                                    class="inline-flex items-center px-4 py-2.5 border border-transparent text-sm font-medium rounded-lg shadow-sm text-white bg-primary-600 hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 disabled:opacity-50 disabled:cursor-not-allowed"
                                >
                            ...
                            <div v-else class="flex items-center justify-between mt-2">
                                <div class="flex items-center text-sm text-green-600 dark:text-green-400">
                                    ...
                                    Promo code applied: {{ cartStore.promoCode.code }}
                                </div>
                                <button
...
const applyPromoCode = async () => {
    ...
    try {
        const result = await cartStore.validatePromoCode(promoCode.value);
        if (!result.success) {
            promoError.value = result.error || 'Invalid promo code';
            toast.error(promoError.value);
        } else {
...
const sendOrderViaWhatsApp = async () => {
    ...
    try {
        ...
        const orderData = {
            ...
            ...(cartStore.promoCode?.id && { promo_code_id: cartStore.promoCode.id }),
        };
