  ...
  <div class="min-h-screen bg-gray-50">
    <div class="container py-8">
      ...
      <div v-else-if="wishlistStore.items.length === 0" class="text-center py-16">
        ...
        <p class="text-gray-500 max-w-md mx-auto mb-8">Discover amazing products and add them to your wishlist for later.</p>
        <div class="flex flex-col sm:flex-row gap-4 justify-center">
          <router-link
            to="/products"
            class="inline-flex items-center px-6 py-3 border border-transparent shadow-sm text-sm font-medium rounded-lg text-white bg-primary-600 hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 transition-all duration-200 transform hover:scale-105"
          >
      ...
      <div v-else class="flex flex-col lg:flex-row gap-8">
        ...
        <div class="lg:w-64 flex-shrink-0">
          <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 sticky top-4">
            <div class="flex items-center justify-between mb-6">
              ...
              <button
                @click="showAddCategoryModal = true"
                class="p-2 bg-primary-100 text-primary-600 rounded-lg hover:bg-primary-200 transition-all duration-200"
            ...
            <div v-if="wishlistStore.defaultCategory" class="mb-4">
              <button
                @click="wishlistStore.selectCategory(wishlistStore.defaultCategory.id)"
                :class="[
                  'w-full text-left px-4 py-3 rounded-lg text-sm font-medium transition-all duration-200 flex items-center',
                  wishlistStore.selectedCategory?.id === wishlistStore.defaultCategory.id
                    ? 'bg-primary-100 text-primary-700 shadow-sm'
                    : 'text-gray-700 hover:bg-gray-50'
                ]"
              >
                <div class="flex items-center flex-1">
                  ...
                  <span class="flex-1">{{ wishlistStore.defaultCategory.name }}</span>
                  <span class="ml-2 text-xs bg-gray-200 text-gray-700 px-2 py-1 rounded-full">
                    {{ wishlistStore.defaultCategory.item_count }}
                  </span>
                </div>
              </button>
            </div>
            ...
            <!-- Custom Categories -->
            <div v-if="wishlistStore.customCategories.length > 0" class="mb-4">
              <h4 class="text-xs font-medium text-gray-500 uppercase tracking-wider mb-3">Custom Categories</h4>
              <div class="space-y-2">
                <div
                ...
                >
                  <button
                    @click="wishlistStore.selectCategory(category.id)"
                    :class="[
                      'w-full text-left px-4 py-3 rounded-lg text-sm font-medium transition-all duration-200 flex items-center',
                      wishlistStore.selectedCategory?.id === category.id
                        ? 'bg-primary-100 text-primary-700 shadow-sm'
                        : 'text-gray-700 hover:bg-gray-50'
                    ]"
                  >
                  ...
                  <!-- Category Actions -->
                  <div class="absolute right-2 top-1/2 transform -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-all duration-200 flex space-x-1">
                    <button
                      @click.stop="editCategory(category)"
                      class="p-1.5 text-gray-400 hover:text-primary-600 bg-white rounded-lg shadow-sm hover:shadow transition-all duration-200"
                      title="Edit category"
                    >
                    ...
                    <button
                      ...
                      class="p-1.5 text-gray-400 hover:text-red-600 bg-white rounded-lg shadow-sm hover:shadow transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
        ...
        <div class="flex-1">
          ...
          <div v-if="wishlistStore.selectedCategory" class="mb-6">
            <div class="flex items-center justify-between bg-white rounded-xl p-6 shadow-sm border border-gray-200">
              <div class="flex items-center">
                ...
                <div>
                  ...
                  <p class="text-sm text-gray-500">
                    {{ wishlistStore.filteredItems.length }} items in this category
                  </p>
                </div>
              </div>
              <div class="flex items-center space-x-3">
                <button
                  ...
                  class="px-4 py-2 text-sm text-gray-700 hover:text-primary-600 border border-gray-300 rounded-lg hover:bg-gray-50 transition-all duration-200 flex items-center"
                >
                ...
                <button
                  @click="clearCategory"
                  class="px-4 py-2 text-sm text-red-700 hover:text-red-800 border border-red-300 rounded-lg hover:bg-red-50 transition-all duration-200 flex items-center"
                >
                  <svg class="h-4 w-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                  </svg>
...
<div class="space-y-4">
  <div
    ...
    :key="item.id"
    class="flex flex-col sm:flex-row items-start sm:items-center bg-white rounded-xl border border-gray-200 shadow-sm hover:shadow-md transition duration-300 p-4"
  >
    <!-- Product Image -->
    <div class="relative w-full sm:w-32 h-40 sm:h-28 flex-shrink-0 overflow-hidden rounded-lg bg-gray-100">
      <router-link
      ...
      <button
        ...
        class="absolute top-2 right-2 p-1.5 bg-white rounded-full shadow hover:bg-gray-100 transition"
      >
    ...
    <div class="flex-1 mt-4 sm:mt-0 sm:ml-5 w-full">
      <!-- Category -->
      <div v-if="!wishlistStore.selectedCategory && item.category" class="mb-1">
      ...
      <!-- Description -->
      <p v-if="item.product?.short_description" class="text-sm text-gray-500 line-clamp-2 sm:line-clamp-1">
      ...
      <div class="mt-2 flex flex-wrap items-center gap-x-2">
        ...
        <span v-if="item.has_product_discount" class="text-xs text-green-600 font-medium">
          Save ${{ formatPrice(item.lowest_price - item.lowest_final_price) }}
        </span>
      </div>
    </div>

    <!-- Actions -->
    <div class="mt-4 sm:mt-0 sm:ml-5 flex space-x-2 w-full sm:w-auto">
      <button
        @click="addToCart(item.product_id)"
        class="flex-1 sm:flex-none bg-primary-600 text-white py-2 px-3 rounded-lg hover:bg-primary-700 transition text-sm font-medium"
      >
        Add to Cart
      </button>
      <button
        @click="moveItemToCategory(item.id)"
        class="p-2 text-gray-400 hover:text-primary-600 bg-gray-50 hover:bg-gray-100 rounded-lg transition"
      ...
      >
        <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div
            v-if="wishlistStore.filteredItems.length === 0 && wishlistStore.selectedCategory"
          ...
          >
            ...
            <button
              @click="showAddCategoryModal = true"
              class="inline-flex items-center px-4 py-2 border border-gray-300 shadow-sm text-sm font-medium rounded-lg text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 transition-all duration-200"
            >
              Add New Category
            </button>
          </div>
      ...
      >
        ...
        <div class="flex space-x-3 mt-8">
          <button
            ...
            :class="!newCategoryName.trim() ? 'opacity-50 cursor-not-allowed' : 'hover:bg-primary-700'"
          >
            Add Category
          </button>
          <button
            @click="showAddCategoryModal = false"
            class="px-4 py-3 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 transition-all"
          >
            Cancel
          </button>
        </div>
        ...
        <div class="space-y-4">
          ...
          <div class="flex items-center">
            <input
              ...
              class="h-4 w-4 text-primary-600 focus:ring-primary-500 border-gray-300 rounded"
              :title="editingCategory?.is_default ? 'This is already the default category' : ''"
            >
    ...
    >
      ...
      >
        <div class="flex items-center justify-between mb-6">
          <h3 class="text-xl font-bold text-gray-900">Move Items to Category</h3>
          <button
            @click="showMoveItemsModal = false"
            class="p-2 text-gray-400 hover:text-gray-600 transition-colors"
          >
        ...
        <div class="space-y-4">
          <div>
            <label for="move-category" class="block text-sm font-medium text-gray-700 mb-2">
              Select Category
            </label>
            <select
            ...
            >
              <option
                ...
                :key="category.id"
                :value="category.id"
                :disabled="category.id === wishlistStore.selectedCategory?.id"
              >
                {{ category.name }}
                <span v-if="category.id === wishlistStore.selectedCategory?.id">(Current)</span>
              </option>
            </select>
          </div>
          ...
          <div class="bg-blue-50 p-4 rounded-lg">
            <p class="text-sm text-blue-800">
              This will move all <span class="font-semibold">{{ wishlistStore.filteredItems.length }}</span> items from
              <span class="font-semibold">{{ wishlistStore.selectedCategory?.name }}</span>
        ...
        <div class="flex space-x-3 mt-8">
          <button
            ...
            :disabled="!selectedMoveCategory || selectedMoveCategory === wishlistStore.selectedCategory?.id"
            class="flex-1 bg-primary-600 text-white py-3 px-4 rounded-lg hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
          >
            Move Items
          </button>
    ...
    >
      ...
      >
        <div class="flex items-center justify-between mb-6">
          <h3 class="text-xl font-bold text-gray-900">Move Item to Category</h3>
          <button
            @click="showMoveItemModal = false"
            class="p-2 text-gray-400 hover:text-gray-600 transition-colors"
          >
        ...
        <div class="space-y-4">
          <div>
            <label for="move-item-category" class="block text-sm font-medium text-gray-700 mb-2">
              Select Category
            </label>
            <select
            ...
            >
              ...
              >
                {{ category.name }}
                <span v-if="category.id === currentItemCategory">(Current)</span>
              </option>
            </select>
          </div>
        </div>
        ...
        <div class="flex space-x-3 mt-8">
          <button
            ...
            :disabled="!selectedMoveItemCategory || selectedMoveItemCategory === currentItemCategory"
            class="flex-1 bg-primary-600 text-white py-3 px-4 rounded-lg hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
          >
            Move Item
          </button>
...
const getProductImageUrl = (item) => {
  if (item.product?.gallery_images && item.product.gallery_images.length > 0) {
...
const addCategory = async () => {
  ...
  const result = await wishlistStore.createCategory(newCategoryName.value, isDefaultCategory.value)
...
const deleteCategory = async (categoryId) => {
  if (!confirm('Are you sure you want to delete this category? All items in this category will be removed.')) {
    return
  }
...
const confirmMoveItem = async () => {
  ...
  try {
    await wishlistStore.moveItem(currentItemToMove.value.id, selectedMoveItemCategory.value)
...
const clearCategory = async () => {
  ...
  if (!confirm(`Are you sure you want to remove all items from ${wishlistStore.selectedCategory.name}?`)) {
    return
  }

  try {
    // Remove each item in the category
    const itemsToRemove = wishlistStore.items.filter(item =>
